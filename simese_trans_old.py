...
class custom_dset(Dataset):
    def __init__(
        ...
        self.nonporeimgs_list = [
            os.path.join(img_path + "\\" + nonpore_folder + "\\", i)
            for i in nonporeimgs
        ]
        ...
        print(
            self.size / len(self.poreimgs_list_shot),
            len(
                self.nonporeimgs_list_shot
                * int(self.size / len(self.nonporeimgs_list_shot))
            ),
            len(self.img1_list),
        )
...
def save_model(
    name, model, poreimgs, nonporeimgs, shot_size, train_size, val_size, test_size
):
    save_dict = {
...
def load_model(name):
    ...
    return (
        ...
        poreimgs[0:shot],
        nonporeimgs[0:shot],
        poreimgs[shot : shot + train_size],
        nonporeimgs[shot : shot + train_size],
        poreimgs[shot + train_size : shot + train_size + test_size],
        nonporeimgs[shot + train_size : shot + train_size + test_size],
        poreimgs[shot + train_size + test_size : final_test_size],
        nonporeimgs[shot + train_size + test_size : final_test_size],
    )
...
if __name__ == "__main__":
    ...
    test_size = min(
        int(np.floor((len(poreimgs_list) - shot - train_size) * 0.5 / 10) * 10),
        int(np.floor((len(nonporeimgs_list) - shot - train_size) * 0.5 / 10) * 10),
    )

    final_test_size = min(
        int(np.floor(len(poreimgs_list) / 10) * 10),
        int(np.floor(len(nonporeimgs_list) / 10) * 10),
    )
    ...
    nonporeimgs_val = nonporeimgs_list[
        train_size + shot : test_size + train_size + shot
    ]
    ...
    feature_encoder_scheduler = StepLR(optimizer, step_size=10000, gamma=0.2)
    ...
    if test_only == 0:
        ...
        for epoch in range(num_epoches):
            ...
            print(
                "Accuracy of the network on the validation images: %0.2f %%"
                % (curr_acc)
            )
            if curr_acc > acc:
                save_model(
            ...
            plt.ylabel("Acc")
            try:
                fig.savefig("plott_acc" + clas + ".png")
            except:
            ...
            plt.ylabel("Loss")
            try:
                fig.savefig("plot_loss" + clas + ".png")
            except:
                print("save failed for some reason")
            plt.close()
