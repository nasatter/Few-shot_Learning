...
class custom_dset(Dataset):
    def __init__(self, img_path, img_transform1, img_transform2, study):
        ...
        self.poreimgs_list = [os.path.join(img_path + "\\pore\\", i) for i in poreimgs]
        self.nonporeimgs_list = [
            os.path.join(img_path + "\\non-pore\\", i) for i in nonporeimgs
        ]
        shuffle1 = np.arange(self.size)
        np.random.shuffle(shuffle1)
        shuffle2 = np.arange(self.size)
        np.random.shuffle(shuffle2)
        self.labels1 = np.concatenate(
            (np.ones(int(self.size / 2)), np.zeros(int(self.size / 2))), axis=0
        )[shuffle1]
        ...
        self.img1_list = np.concatenate(
            (
                self.poreimgs_list[0 : int(self.size / 2)],
                self.nonporeimgs_list[0 : int(self.size / 2)],
            ),
            axis=0,
        )[shuffle1]
        self.img2_list = np.concatenate(
            (
                self.poreimgs_list[int(self.size / 2) :],
                self.nonporeimgs_list[int(self.size / 2) :],
            ),
            axis=0,
        )[shuffle2]
...
def load_images_to_memory(img_path):
    ...
    poreimgs = os.listdir(img_path + "\\pore")
    nonporeimgs = os.listdir(img_path + "\\non-pore")
    poreimgs_list = [os.path.join(img_path + "\\pore\\", i) for i in poreimgs]
    nonporeimgs_list = [os.path.join(img_path + "\\non-pore\\", i) for i in nonporeimgs]
...
if __name__ == "__main__":
    ...
    if test_only == 0:
        ...
        for epoch in range(num_epoches):
            ...
            print("Accuracy of the network on the test images: %0.2f %%" % (curr_acc))
            if curr_acc > acc:
            ...
            for j in range(dist.size()[0]):
                if dist.data.numpy()[j] < 0.5:
                    ...
                    if labels.cpu().data.numpy()[j] == 0:
                        ...
                        total += 1
        print(
            "Accuracy of the network on the train images: %d %%"
            % (100 * correct / total)
        )
